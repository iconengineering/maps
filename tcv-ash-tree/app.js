...
firebase.auth().onAuthStateChanged(function(user) {
  if (user) {
    ...
    var ref = firebase.database().ref("datacollector/users/" + displayName + "/write/tcvAshTree");
    ref.once("value")
...
var map = new mapboxgl.Map({
  ...
  center: [-105.031,39.601], // starting position
...
map.on('draw.create', function() {
  ...
  actionButton.addEventListener('click', function(){
    ...
    // change per project
    var storageRef = firebase.storage().ref('tcvAshTreeUpload/');
      ...
      snapshot.forEach(function(feature) {
       ...
       //   map.setLayoutProperty('firebasePoly', 'visibility', 'visible');
        ...
        deleteButton.addEventListener('click', function(){
          ...
          // change per project
          firebase.database().ref('datacollector/tcvAshTree/' + featureKey).remove(function(error) {
            if (error) {
              Materialize.toast('Something went wrong.');
          } else {
