...
firebase.auth().onAuthStateChanged(function(user) {
  if (user) {
    ...
    var ref = firebase.database().ref("datacollector/users/" + displayName + "/write/greeley");
    ref.once("value")
...
var map = new mapboxgl.Map({
  ...
  center: [-104.705, 40.42], // starting position
...
var dataRef = firebase.database().ref('datacollector/greeley');
var archiveRef = firebase.database().ref('datacollector/greeleyArchive');
...
map.on('draw.create', function() {
  ...
  actionButton.addEventListener('click', function(){
    ...
    var storageRef = firebase.storage().ref('greeleyUpload/');
    ...
    dataRef.orderByChild('id').equalTo(editFeature.id).once('value', function(snapshot) {
      snapshot.forEach(function(feature) {
        ...
        actionButton.addEventListener('click', function(){
          ...
          firebase.database().ref('datacollector/greeley/' + featureKey).update(draw.getSelected().features[0], function(error) {
            if (error) {
              Materialize.toast('Something went wrong.');
          } else {
        ...
        deleteButton.addEventListener('click', function(){
          ...
          firebase.database().ref('datacollector/greeley/' + featureKey).remove(function(error) {
            if (error) {
              Materialize.toast('Something went wrong.');
          } else {
